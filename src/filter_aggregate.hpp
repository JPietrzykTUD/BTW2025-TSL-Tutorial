...
auto filter_aggregate_differing_parallelism_impl(std::span<const AggregateColumnT> to_agg, std::span<const FilterColumnT> to_filter, FilterColumnT const filter_value) -> AggregateColumnT {
...
auto filter_aggregate(std::span<const AggregateColumnT> to_agg, std::span<const FilterColumnT> to_filter, FilterColumnT const filter_value) {
  ...
  if constexpr (parallelism_available_for_all<SimdPar, FilterColumnT, AggregateColumnT>()) {
    ...
    result = filter_aggregate_impl<SimdPar, FilterColumnT, AggregateColumnT>(to_agg_simd, to_filter_simd, filter_value);

  } else {
    std::cout << "Executing filter_aggregate_differing_parallelism_impl on [0:" << to_filter_simd_count <<")" << std::endl;
    result = filter_aggregate_differing_parallelism_impl<SimdPar, FilterColumnT, simd_helper::max_par<AggregateColumnT>(), AggregateColumnT>(to_agg_simd, to_filter_simd, filter_value);
  }
